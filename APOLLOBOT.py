...
class IqOptionBotBackEnd:
    ...
    def __init__(self):
        ...
        self.interface = uic.loadUi("/home/andows/worldofsaphael/iq option bot/layout01.ui")
        self.interface.conectar.clicked.connect(self.fazer_login_interface)
        self.interface.efetuar.clicked.connect(self.dar_entrada_interface)
        self.interface.show()
        app.exec()
   ...
   # coleta os dados que serão inseridos na "dar_entrada()"
    ...
    def dar_entrada_digital(self,):
        status,id = self.API.buy_digital_spot("EURUSD", 10, "call", 1)
    ...
    def dar_entrada_binaria(self):
        self.status,self.id = self.API.buy(10, 'EURAUD', 'put', 1)
        
        
    def conferir_entrada_binaria(self):
        if self.status:
            print("Comprada")
            resultado,lucro = self.API.check_win_v3(self.id)
    ...
    def fazer_login_interface(self):
        email = self.interface.email.text()
        senha = self.interface.senha.text()
        
        self.fazer_login(email, senha )
        
    
    def dar_entrada_interface(self):
        print("Efetuado")
        self.API.change_balance('PRACTICE')
        self.dar_entrada_binaria()
        self.conferir_entrada_binaria()
